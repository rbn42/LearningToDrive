...
class Demo(ShowBase):
   ...
   # stereo = cv2.StereoBM_create(numDisparities=16, blockSize=15)
    ...
    def __init__(self, img_size=512, screen_off=True,target_area_radius=5,initial_area_radius=10, keyboard_input=False,random_reset_around_target =False,test=False):
        logging.info('random_reset_around_target :%s',random_reset_around_target )
...
#        self.cam2 = camNP  # Camera('cam')p
      ...
      #  cnodePath.show()
        ...
        cnodePath = self.navigation_target.attachNewNode(CollisionNode('target'))
    ...
    def step(self, action=(0, 0, 0)): 
        ...
        if not None == self.relative_target_position:
            speed = abs(relative_target_position) - abs(self.relative_target_position)
    ...
    def resetGame(self): 
        ...
        while True:
            if not self.random_reset_around_target: #:
                ...
                while True:
                    pos = (10 + size * math.pi * rng.rand()) * np.exp(rng.rand() * math.pi * 1j)
                    pos += target_pos
        ...
        for p in self.pieces:
            p.reset(avoids=[(self.agent.getPos(),self.initial_area_radius),(self.navigation_target.getPos(),self.target_area_radius)])
        # print(i)
...
class Piece:
    ...
    def reset(self,avoids=[]):
        ...
        while True:
            x,y =rng.uniform(-self.roomsize,self.roomsize,2)# rng.rand() * 2 * self.roomsize - self.roomsize        y = rng.rand() * 2 * self.roomsize - self.roomsize
    ...
    def step(self, dt):
        dx,dy =rng.uniform(- self.acceleration_max,self.acceleration_max,2)# * (2 * rng.rand() - 1)        dy = self.acceleration_max * (2 * rng.rand() - 1)
...
# #    imageObject.reparentTo(win2Render2d)
   ...
   #     print(demo.relative_target_position) 
        action_options = [ (0, 0, 0.1),(2, 0, 0.1), (-2, 0, 0.1),(0,0,0) ]  # (0,0,0)]
        if self.keyMap["left"]:
